import{a as M}from"./wii.QxhGZP-6.js";import{q as N,c as O,a as x,f as q,e as P,d as I,u as L}from"./firebase.Ba2Rv1m_.js";import{p as U}from"./auth._HsHFvv8.js";import{getNombre as F,getls as G,savels as B,Notificacion as g}from"./widev.Cj3mzJXC.js";let h=null,c=[],f="pendientes",S="todos",k="",v=null;const w=.5,$="ge_cache_solicitudes";let T=!1;const R=t=>{if(!t)return"—";const a=new Date(t);return isNaN(a.getTime())?"—":a.toLocaleDateString("es-ES",{day:"numeric",month:"short",year:"numeric"})+" a las "+a.toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"})},J=()=>c.filter(t=>{if(f==="pendientes"){if(t.estado!=="pendiente")return!1}else if(t.estado==="pendiente"||S!=="todos"&&t.estado!==S)return!1;if(k.trim()){const a=k.toLowerCase().trim(),o=`${t.nombre||""} ${t.apellidos||""}`.toLowerCase(),s=(t.usuario||"").toLowerCase(),n=(t.email||"").toLowerCase();if(!o.includes(a)&&!s.includes(a)&&!n.includes(a))return!1}return!0}),E=()=>{const t=c.filter(r=>r.estado==="pendiente").length,a=c.filter(r=>r.estado==="aprobado").length,o=c.filter(r=>r.estado==="rechazado").length,s=document.getElementById("stat_pending"),n=document.getElementById("stat_approved"),e=document.getElementById("stat_rejected"),i=document.getElementById("badge_pending"),d=document.getElementById("badge_history");s&&(s.textContent=String(t)),n&&(n.textContent=String(a)),e&&(e.textContent=String(o)),i&&(i.textContent=String(t)),d&&(d.textContent=String(a+o))},u=()=>{const t=J(),a=document.getElementById("gestor_table_body"),o=document.getElementById("gestor_mobile_list"),s=document.getElementById("gestor_empty"),n=document.getElementById("table_actions_header");if(n&&(n.textContent=f==="pendientes"?"Acciones":"Resolución"),t.length===0){if(a&&(a.innerHTML=""),o&&(o.innerHTML=""),s){s.classList.remove("dpn");const e=document.getElementById("empty_title"),i=document.getElementById("empty_desc");e&&i&&(f==="pendientes"?(e.textContent="No hay solicitudes pendientes",i.textContent="No tienes postulaciones pendientes de evaluar por el momento."):(e.textContent="Historial vacío",i.textContent="No se registran postulaciones evaluadas bajo los filtros actuales."))}return}s&&s.classList.add("dpn"),a&&(a.innerHTML=t.map(e=>{const i=`${e.nombre||""} ${e.apellidos||""}`.trim()||e.usuario||"Usuario",d=R(e.creado);let r="";return f==="pendientes"?r=`
          ...
          <tr>
            <td>
              <div class="gestor_user_info">
                <span class="gestor_user_name">${i}</span>
        ...
        `}).join("")),o&&(o.innerHTML=t.map(e=>{const i=`${e.nombre||""} ${e.apellidos||""}`.trim()||e.usuario||"Usuario",d=R(e.creado);let r="";return f==="pendientes"?r=`
