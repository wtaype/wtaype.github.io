import{j as a}from"./vendor-gzd0YkcT.js";import{d as v}from"./firebase-WhHZvOPg.js";import{s as k,d as w,a as M,g as C,q as x,c as $,o as O,T as I,b as q}from"./firebase-HZkv0mxq.js";import{g as E,b as L,e as N,d as P,N as n,w as S,f as G,s as R,h as z,M as F}from"./main-DbUXgSKj.js";const T="proyectos",U=4,l=8,j=["web|Web Apps|globe","mobile|Mobile|mobile-alt","windows|Windows|windows","educacion|Educación|graduation-cap","diseno|Diseño|palette","devs|Devs|user-astronaut","tools|Herramientas|tools","camino|Camino|hourglass-half"],W=["destacado|Destacados","reciente|Más reciente","antiguo|Más antiguo","nombre|Por nombre"],B=[{titulo:"DSCTO - Calculadora Móvil",img:"https://i.postimg.cc/dq8nVhCx/Dscto.png",descripcion:"App para cálculos rápidos de descuentos y ajustes",url:"https://dscto.blogspot.com/",tags:["JavaScript","PWA","Firebase"]},{titulo:"CODEWIL - Optimizador de Código",img:"https://i.postimg.cc/CSQcPTYm/Codewil.png",descripcion:"Herramienta para optimizar y embellecer código",url:"https://codewil.blogspot.com/",tags:["React","Node.js","API"]},{titulo:"WICOLORS - Paleta de Colores",img:"https://i.postimg.cc/FhkDwWHm/Wiicolors.png",descripcion:"Generador inteligente de paletas de colores",url:"https://wicolors.blogspot.com/",tags:["Vue.js","Design","CSS"]}],_=z(I);let i=[],r=[],p=0,g=!1,y=null,b=[];const m=o=>(o||"").toString().toLowerCase(),H=o=>o.split(",").map(t=>t.trim()).filter(Boolean),f=o=>+(o.fechaProyecto?.toDate?.()||o.UpdateEn?.toDate?.()||0),V=o=>{const t=_?.get?.(o,"date")||_?.get?.(o,"full")||"";if(/^\d{4}-\d{2}-\d{2}/.test(t))return t.slice(0,10);const e=o?.toDate?.()||typeof o=="string"&&new Date(o);return e&&!isNaN(e)?e.toISOString().slice(0,10):""},J=o=>_?.save?.(o)||I.fromDate(new Date(o?`${o}T00:00:00`:Date.now())),Z=()=>`
...
<div class="wiModal" id="modalProyecto">
  <div class="modalBody modal_proyecto_body">
    ...
    <form class="modal_proyecto_form" id="formProyecto">
...
</div>`,aa=()=>{i=[],r=[],p=0,g=!!E("wiSmile"),setTimeout(()=>{a("#destacadosGrid").html(B.map((o,t)=>`
      ...
      </div>`)}),setTimeout(()=>e.find(".proyecto_card:not(.visible)").addClass("visible"),50),p+=o,p<r.length?a("#loadMoreSection").fadeIn(200):a("#loadMoreSection").fadeOut(200)},h=async(o=!1)=>{if(!o){const t=(E(T)||{}).data||[];if(t.length){i=r=t,c(l,!0);return}}try{let t;try{t=await C(x($(v,"proyectos"),O("creadoEn","desc")))}catch{t=await C($(v,"proyectos"))}i=r=t.docs.map(e=>({id:e.id,...e.data()})),c(l,!0),R(T,{data:i,ts:Date.now()},24),a("#buscarProyecto").attr("placeholder",`Buscar en ${i.length} proyectos...`)}catch(t){i.length||a(".loading_grid").html('<i class="fas fa-exclamation-triangle"></i><p>Error al cargar</p>'),console.error("❌ proyectos:",t)}};export{oa as cleanup,aa as init,Z as render};
