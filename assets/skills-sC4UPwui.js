...
<div class="skills_container">
  ...
  <section class="skills_filtros">
    <div class="filtros_wrapper">
      ${v.map(([o,a,e])=>`<button class="categoria_btn${o==="all"?" active":""}" data-categoria="${o}"><i class="${a}"></i><span>${e}</span></button>`).join("")}
    </div>
  </section>
  ...
  <section class="tech_principales">
    ...
    <div class="tech_grid">
...
</div>`,$=()=>{p(s),i(document).on("click.skills",".categoria_btn",function(){const o=i(this).data("categoria");i(".categoria_btn").removeClass("active"),i(this).addClass("active");const a=o==="all"?s:s.filter(e=>e.categoria===o);p(a)})},F=()=>{i(document).off(".skills"),t.forEach(o=>o?.disconnect?.()),t=[]},p=o=>{const a=i("#skillsGrid").empty();o.forEach((e,c)=>{a.append(`
      ...
      </div>`)}),j()},j=()=>{const o=m(".skills_grid",()=>{i(".skill_card").each(function(a){const e=i(this),c=e.find(".circle_progress"),l=e.find(".porcentaje_numero"),r=parseInt(c.data("porcentaje"));setTimeout(()=>{e.addClass("visible");const d=2*Math.PI*45;c.css("stroke-dashoffset",d-r/100*d);let n=0;const b=setInterval(()=>{n+=r/60,n>=r?(l.text(r),clearInterval(b)):l.text(Math.floor(n))},25)},a*50)})});t.push(o)};export{F as cleanup,$ as init,C as render};
