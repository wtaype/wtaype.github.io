      ...
      <div class="info_section">
        ...
        <div class="info_card">
          ...
          <div class="redes_grid">
            ${f.map(e=>`<a href="${e.url}" target="_blank" rel="noopener" class="red_social" style="--color:${e.color}"><i class="${e.icono}"></i><span>${e.nombre}</span></a>`).join("")}
          </div>
        </div>
      </div>
...
</div>`,_=()=>{a(document).on("click.contacto","#ctaIniciar",()=>{a("#nombre").trigger("focus"),a("html,body").animate({scrollTop:a("#contactoForm").offset()?.top-80},400)}),a(document).on("input.contacto","#mensaje",function(){const e=Math.min(a(this).val().length,t);a(this).val().length>t&&a(this).val(a(this).val().slice(0,t)),a("#charCount").text(e)}),a(document).on("reset.contacto","#contactoForm",()=>{a("#charCount").text("0"),s("Formulario limpiado","info",2e3)}),a(document).on("submit.contacto","#contactoForm",async function(e){e.preventDefault();const o={nombre:a("#nombre").val().trim(),email:a("#email").val().trim(),telefono:a("#telefono").val().trim(),asunto:a("#asunto").val(),mensaje:a("#mensaje").val().trim(),fecha:new Date().toISOString()};if(o.nombre.length<3)return s("El nombre debe tener al menos 3 caracteres","error");if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(o.email))return s("Ingresa un email válido","error");if(o.mensaje.length<10)return s("El mensaje debe tener al menos 10 caracteres","error");const c=a(".btn_submit");r(c,!0,"Enviando...");try{await new Promise(l=>setTimeout(l,2e3)),s("¡Mensaje enviado con éxito! Te responderé pronto.","success",4e3),this.reset(),a("#charCount").text("0")}catch(l){console.error("Error contacto:",l),s("Error al enviar el mensaje. Intenta nuevamente.","error")}finally{r(c,!1,"Enviar Mensaje")}}),a(document).on("click.contacto",".btn_copy",function(){m(a(this).data("copy"),this,"¡Copiado!")}),a(document).on("click.contacto",".faq_question",function(){const e=a(this).closest(".faq_item"),o=e.hasClass("active");a(".faq_item").not(e[0]).removeClass("active").find(".faq_answer").slideUp(300),a(".faq_item").not(e[0]).find(".faq_toggle").removeClass("rotated"),o?(e.removeClass("active").find(".faq_answer").slideUp(300),a(this).find(".faq_toggle").removeClass("rotated")):(e.addClass("active").find(".faq_answer").slideDown(300),a(this).find(".faq_toggle").addClass("rotated"))}),n.push(d(".info_card",(e,o)=>setTimeout(()=>a(e).addClass("visible"),o*150))),n.push(d(".faq_item",(e,o)=>setTimeout(()=>a(e).addClass("visible"),o*100)))},h=()=>{a(document).off(".contacto"),n.forEach(e=>e?.disconnect?.()),n=[]};export{h as cleanup,_ as init,b as render};
